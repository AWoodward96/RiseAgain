...
@export_file() var base_palette
@export_file() var in_palettes_dir
@export_file() var out_palettes_dir
...
func _ready():
	...
	var dir = DirAccess.open(in_palettes_dir)
	dir.list_dir_begin()
	...
	var save = FileAccess.open(out_palettes_dir + "data.json", FileAccess.WRITE)
	if save != null:
		save.store_string(JSON.stringify(colorData))
		save.close()
	else:
...
func createPalette(file:String, centers:Array):
	var image = Image.new()
	image.load(in_palettes_dir + file)
	...
	out.save_png(out_palettes_dir+file.substr(0, file.find("."))+".png")
